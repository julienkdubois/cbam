...
var
    ...
    %--- SHOCKS ---
    e_z_H e_z_F  % TFP shock
    e_p_H e_p_F  % Cost-push shock
    e_r_H e_r_F  % Monetary policy shock
    e_x_H e_x_F  % Import demand shock
    e_t_H e_t_F  % Carbon tax shock
    e_e          % UIP shock
    ;
...
% Local variables
% ----------------------------------------------------------------
% Nominal exchange rate: e = rer * cpi_H / cpi_F  (exact relationship)
#e_t = rer * cpi_H / cpi_F;
...
% Export share of a representative foreign firm
...
% Effective elasticity for foreign firm
...
theta1_F * theta2_F * mu_F^(theta2_F-1) = tau_F * sig_F + eta * chi_F * ((p_int_F - mc_F) / p_tilde_F) * CBAM_t * sig_F;
...
[name='NKPC, Foreign']
kappa_F * (pi_F - pi_star) * pi_F = (1-eta_eff_F) * (p_int_F / PG_F) + eta_eff_F * e_p_F * mc_F + beta * kappa_F * (pi_F(+1) - pi_star) * pi_F(+1) * y_int_F(+1) / y_int_F;
...
T_H = tau_H * e_H + e_t * CBAM_t * sig_F * (1-mu_F) * gamma_y_H * l_F * (e_t * p_tilde_F / PG_H)^(-eta) * G_H;
...
PG_H = ( (1-gamma_y_H)*l_H*p_int_H^(1-eta) + gamma_y_H*l_F*(e_ss*p_tilde_F_ss)^(1-eta) )^(1/(1-eta));
PG_F = ( gamma_y_F*l_H*((1+tau_i)*p_int_H/e_ss)^(1-eta) + (1-gamma_y_F)*l_F*p_int_F^(1-eta) )^(1/(1-eta));

% --- 8. Home marginal cost ---
mc_H = (eta-1)/eta;
...
% --- 13. Consumption from goods market clearing ---
...
stoch_simul(order=1, irf=20) y_H y_F c_H c_F pi_H pi_F cpi_H cpi_F r_H r_F rer ex_H ex_F e_H e_F;
